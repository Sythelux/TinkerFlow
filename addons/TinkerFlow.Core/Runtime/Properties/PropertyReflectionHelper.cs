...
internal static class PropertyReflectionHelper
{
    ...
    {
        List<LockablePropertyData> result = new();
    ...
    {
        List<ISceneObjectProperty> result = new();

        List<MemberInfo> memberInfo = GetAllPropertyReferencesFromCondition(data);
        memberInfo.ForEach(info =>
        {
            var reference = ReflectionUtils.GetValueFromPropertyOrField(data, info) as UniqueNameReference;

            if (reference == null || string.IsNullOrEmpty(reference.UniqueName)) return;

            if (RuntimeConfigurator.Configuration.SceneObjectRegistry.ContainsName(reference.UniqueName) == false) return;
            ...
            if (refType != null)
            {
                ...
                if (checkRequiredComponentsToo) types = GetDependenciesFrom(refType);

                foreach (Type type in types)
                {
                    var property = GetFittingPropertyFromReference<ISceneObjectProperty>(reference, type);
    ...
    public static List<LockablePropertyData> ExtractLockablePropertiesFromConditionTags(IConditionData data, bool checkRequiredComponentsToo = true)
    {
        ...
        memberInfo.ForEach(info =>
        {
            var reference = ReflectionUtils.GetValueFromPropertyOrField(data, info) as SceneObjectTagBase;

            if (reference == null || reference.Guid == null || reference.Guid == Guid.Empty) return;

            if (SceneObjectTags.Instance.TagExists(reference.Guid) == false) return;

            IEnumerable<ISceneObject> taggedObjects = RuntimeConfigurator.Configuration.SceneObjectRegistry.GetByTag(reference.Guid);

            if (taggedObjects.Count() == 0) return;
            ...
            foreach (ISceneObject taggedObject in taggedObjects)
            {
                ...
                {
                    IEnumerable<Type> types = new[] { refType };
                    if (checkRequiredComponentsToo) types = GetDependenciesFrom<LockableProperty>(refType);

                    foreach (Type type in types)
                    {
                        var property = taggedObject.Properties.FirstOrDefault(property => property.GetType() == type) as LockableProperty;
                        if (property != null) result.Add(new LockablePropertyData(property));
                    }
                }
            }
        });
    ...
    public static List<LockablePropertyData> ExtractLockablePropertiesFromConditions(IConditionData data, bool checkRequiredComponentsToo = true)
    {
        ...
        memberInfo.ForEach(info =>
        {
            ...
            if (refType != null)
            {
                ...
                foreach (Type type in types)
                {
                    var property = GetFittingPropertyFromReference<LockableProperty>(reference, type);
                    if (property != null) result.Add(new LockablePropertyData(property));
                }
            }
        });
    ...
    private static T GetFittingPropertyFromReference<T>(UniqueNameReference reference, Type type) where T : class, ISceneObjectProperty
    {
        ...
        foreach (ISceneObjectProperty prop in sceneObject.Properties)
            if (prop.GetType() == type)
                return prop as T;
        GD.PushWarning("Could not find fitting {0} type in SceneObject {1}", type.Name, reference.UniqueName);
        return null;
    }
