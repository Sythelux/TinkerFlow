...
public abstract partial class LockableProperty : ProcessSceneObjectProperty, ILockable
{
    ...
    {
        if (lockState == false && stepData != null && unlockers.Contains(stepData) == false) unlockers.Add(stepData);

        if (lockState && stepData != null && unlockers.Contains(stepData)) unlockers.Remove(stepData);

        bool canLock = unlockers.Count == 0;

        // TODO: if (LifeCycleLoggingConfig.Instance.LogLockState)
        // {
        ...
        //
        //     GD.Print($"<i>{this.GetType().Name}</i> on <i>{gameObject.name}</i> received a <b>{lockType}</b> request from <i>{requester}</i>." +
    ...
    {
        if (LockOnParentObjectLock && IsLocked) SetLocked(false);
    }

    private void HandleObjectLocked(object sender, LockStateChangedEventArgs e)
    {
        if (LockOnParentObjectLock && IsLocked == false) SetLocked(true);
    }
