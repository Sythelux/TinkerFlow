...
{
    [DataMember]
    private Dictionary<string, Dictionary<string, object>> values = new();
    ...
    {
        if (values.ContainsKey(member.Name) == false) values[member.Name] = new Dictionary<string, object>();
    ...
    {
        if (values.ContainsKey(member.Name) && values[member.Name].ContainsKey(attribute.Name)) return values[member.Name][attribute.Name];
    ...
    {
        if (values.ContainsKey(member.Name)) return values[member.Name].ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
