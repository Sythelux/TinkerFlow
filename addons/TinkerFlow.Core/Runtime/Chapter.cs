...
public class Chapter : Entity<Chapter.EntityData>, IChapter
{
    ...
    private class ActivatingProcess : EntityIteratingProcess<IEntitySequenceDataWithMode<IStep>, IStep>
    {
        ...
        public override void FastForward()
        {
            ...
            if (Data.Current.FindPathInGraph(step => step.Data.Transitions.Data.Transitions.Select(transition => transition.Data.TargetStep), null, out IList<IStep> pathToChapterEnd) == false) throw new InvalidStateException("The end of the chapter is not reachable from the current step.");

            foreach (IStep step in pathToChapterEnd)
            {
                if (Data.Current.LifeCycle.Stage == Stage.Inactive) Data.Current.LifeCycle.Activate();
                ...
                if (toAutocomplete.IsCompleted == false) toAutocomplete.Autocomplete();
    ...
    public IChapter Clone()
    {
        ...
        Dictionary<IStep, IStep> clonedSteps = new();

        foreach (IStep step in Data.Steps)
        {
            IStep clonedStep = step.Clone();
            clonedChapter.Data.Steps.Add(clonedStep);
            if (Data.FirstStep == step) clonedChapter.Data.FirstStep = clonedStep;
    ...
    public Chapter(string name, IStep firstStep)
    {
        ...
        if (firstStep != null) Data.Steps.Add(firstStep);

        if (LifeCycleLoggingConfig.Instance.LogChapters)
            LifeCycle.StageChanged += (sender, args) => { GD.Print($"<b>Chapter</b> <i>'{Data.Name}'</i> is <b>{LifeCycle.Stage.ToString()}</b>.\n"); };
    }
}