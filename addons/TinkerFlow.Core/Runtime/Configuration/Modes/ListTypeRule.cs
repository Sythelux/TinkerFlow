...
{
    private HashSet<Type> storedTypes = new();

    protected HashSet<Type> StoredTypes => storedTypes;
    ...
    public TRecursive Add<T>() where T : TValueBase
    {
        ...
        if (result.storedTypes.Contains(typeof(T))) return result;
    ...
    {
        var result = new TRecursive { storedTypes = new HashSet<Type>(storedTypes) };
        return result;
    }
}