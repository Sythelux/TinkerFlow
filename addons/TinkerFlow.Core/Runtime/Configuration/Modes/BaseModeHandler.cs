...
public sealed class BaseModeHandler : IModeHandler
{
    ...
    /// <inheritdoc />
    public IMode CurrentMode => AvailableModes[CurrentModeIndex];
    ...
    {
        if (AvailableModes.Count == 0) throw new MissingModeException("You cannot access the current process mode index because there are no process modes available.");
        ...
        if (ModeChanged != null) ModeChanged(this, new ModeChangedEventArgs(CurrentMode));
    }
