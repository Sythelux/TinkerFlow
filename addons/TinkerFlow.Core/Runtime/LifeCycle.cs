...
public sealed class LifeCycle : ILifeCycle
{
    ...
    private readonly Dictionary<Stage, bool> fastForwardedStates = new()
    {
    ...
    {
        if (Stage != Stage.Inactive) throw new InvalidStateException("Process entity can only be activated when not running yet. Stage: " + Stage);
    ...
    public void MarkToFastForwardStage(Stage stage)
    {
        ...
        if (stage == Stage) FastForward();
    }

    ///<inheritdoc />
    public void Update()
    {
        if (IsCurrentStageProcessFinished) return;

        if (update.MoveNext() == false) FinishCurrentState();
    }
    ...
    private void StartActivating()
    {
        ...
        if (IsInFastForward) FastForward();
    }
    ...
    private void StartActive()
    {
        ...
        if (deactivateAfterActivation) Deactivate();
    }
    ...
    private bool IsInFastForward => fastForwardedStates[Stage];
