...
public abstract class EntityIteratingProcess<TEntitySequenceDataWithMode, TEntity> : StageProcess<TEntitySequenceDataWithMode> where TEntity : IEntity where TEntitySequenceDataWithMode : class, IEntitySequenceDataWithMode<TEntity>
{
    ...
    public override IEnumerator Update()
    {
        ...
        while (TryNext(out current))
        {
            ...
            while (ShouldActivateCurrent() == false) yield return null;

            Data.Current.LifeCycle.Activate();

            if (Data.Current is IOptional && Data.Mode.CheckIfSkipped(Data.Current.GetType())) Data.Current.LifeCycle.MarkToFastForward();

            while (current.LifeCycle.Stage == Stage.Activating) yield return null;

            while (ShouldDeactivateCurrent() == false) yield return null;

            if (Data.Current.LifeCycle.Stage != Stage.Inactive) Data.Current.LifeCycle.Deactivate();

            while (Data.Current.LifeCycle.Stage != Stage.Inactive) yield return null;
        }
    }
    ...
    public override void FastForward()
    {
        ...
        while (current != null || TryNext(out current))
        {
            ...
            if (current.LifeCycle.Stage == Stage.Inactive) current.LifeCycle.Activate();

            current.LifeCycle.MarkToFastForward();

            if (current.LifeCycle.Stage == Stage.Activating || current.LifeCycle.Stage == Stage.Active) current.LifeCycle.Deactivate();
