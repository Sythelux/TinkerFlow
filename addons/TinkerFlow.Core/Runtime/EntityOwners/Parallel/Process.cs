...
internal abstract class Process<TData> : StageProcess<TData> where TData : class, IEntityCollectionData, IModeData
{
    /// <summary>
    ...
    protected IEnumerable<IEntity> GetBlockingChildren(IEntityCollectionData collection, IMode mode)
    {
        return collection.GetChildren()
            ...
            .Where(child =>
            {
                var dataOwner = child as IDataOwner;
                if (dataOwner == null) return true;

                var blockingData = dataOwner.Data as IBackgroundBehaviorData;
