    ...
    public override void Configure(IMode mode, Stage stage)
    {
        foreach (TEntity child in Data.GetChildren())
        {
            if (child is IOptional)
            {
                ...
                if (wasSkipped == isSkipped) continue;

                if (isSkipped)
                {
                    if (child.LifeCycle.Stage == Stage.Inactive) continue;

                    child.LifeCycle.MarkToFastForward();

                    if (child.LifeCycle.Stage == Stage.Active) child.LifeCycle.Deactivate();
                }
                else
                {
                    ...
                    if (stage == Stage.Activating || stage == Stage.Active) child.LifeCycle.Activate();
                }
            }
