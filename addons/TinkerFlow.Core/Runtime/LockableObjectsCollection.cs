...
internal class LockableObjectsCollection
{
    ...
    private void CreateSceneObjects()
    {
        ...
        foreach (LockablePropertyReference propertyReference in data.ToUnlock) AddSceneObject(propertyReference.Target.Value);

        foreach (LockablePropertyData propertyData in toUnlock) AddSceneObject(propertyData.Property.SceneObject);
    }
    ...
    public void RemoveSceneObject(ISceneObject sceneObject)
    {
        if (SceneObjects.Remove(sceneObject))
            ...
            {
                if (property.GetProperty() == null) return false;
    ...
    {
        if (data.TagsToUnlock.ContainsKey(tag)) return;
    ...
    {
        if (data.TagsToUnlock.ContainsKey(tag) == false) return;
