...
public class NewtonsoftJsonProcessSerializerV4 : NewtonsoftJsonProcessSerializer
{
    ...
    public override IProcess ProcessFromByteArray(byte[] data)
    {
        ...
        var dataObject = JsonConvert.DeserializeObject<JObject>(stringData, ProcessSerializerSettings);

        // Check if process was serialized with a previous version.
        var version = dataObject.GetValue("$serializerVersion").ToObject<int>();
        if (version == 1) return base.ProcessFromByteArray(data);
        if (version == 2) return new ImprovedNewtonsoftJsonProcessSerializer().ProcessFromByteArray(data);
        if (version == 3) return new NewtonsoftJsonProcessSerializerV3().ProcessFromByteArray(data);

        var wrapper = Deserialize<ProcessWrapper>(data, ProcessSerializerSettings);
    ...
    public override IChapter ChapterFromByteArray(byte[] data)
    {
        ...
        var version = dataObject.GetValue("$serializerVersion").ToObject<int>();
        if (version == 1) return base.ChapterFromByteArray(data);
        if (version == 2) return new ImprovedNewtonsoftJsonProcessSerializer().ChapterFromByteArray(data);
        if (version == 3) return new NewtonsoftJsonProcessSerializerV3().ChapterFromByteArray(data);

        var wrapper = Deserialize<ChapterWrapper>(data, ProcessSerializerSettings);
    ...
    {
        var wrapper = new ChapterWrapper(chapter);
        byte[] bytes = null;
