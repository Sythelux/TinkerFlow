...
public class NewtonsoftJsonProcessSerializer : IProcessSerializer
{
    ...
    /// </summary>
    public static JsonSerializerSettings ProcessSerializerSettings => CreateSettings(GetJsonConverters());

    private static JsonSerializerSettings StepSerializerSettings
    {
        get
        {
            List<JsonConverter> converters = new() { new IndividualStepTransitionConverter() };
    ...
    {
        var dataObject = JsonConvert.DeserializeObject<JObject>(new UTF8Encoding().GetString(data), ProcessSerializerSettings);

        // Check if process was serialized with version 1
        var version = dataObject.GetValue("$serializerVersion").ToObject<int>();
        if (version != 1) throw new Exception($"The loaded process is serialized with a serializer version {version}, which in compatible with this serializer.");
    ...
    internal class ProcessSerializationBinder : DefaultSerializationBinder
    {
        ...
        {
            if (typeName == "VRBuilder.Editor.UI.Drawers.Metadata.ReorderableElementMetadata") return typeof(ReorderableElementMetadata);
