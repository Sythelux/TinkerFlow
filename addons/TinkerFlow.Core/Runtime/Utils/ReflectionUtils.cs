...
public static class ReflectionUtils
{
    ...
    {
        if (type.MemberType == MemberTypes.NestedType) return string.Concat(GetNameWithNesting(type.DeclaringType), "+", type.Name);
    ...
    public static Type GetEntryType(object list)
    {
        ...
        if (genericListType != null) entryDeclaredType = genericListType.GetGenericArguments()[0];
    ...
    public static void RemoveFromList(ref IList list, int index)
    {
        if (list.IsFixedSize)
        {
            var temp = (IList)Activator.CreateInstance(list.GetType(), list.Count - 1);
            for (var j = 0; j < index; j++) temp[j] = list[j];

            for (int j = index + 1; j < list.Count; j++) temp[j - 1] = list[j];
    ...
    public static void ReplaceList<T>(ref IList list, IEnumerable<T> newList)
    {
        ...
        var result = (IList)Activator.CreateInstance(list.GetType(), newArray.Length);
    ...
    {
        if (IsProperty(info)) return ((PropertyInfo)info).PropertyType;

        if (IsField(info)) return ((FieldInfo)info).FieldType;
    ...
    public static void SetValueToPropertyOrField(object owner, MemberInfo member, object value)
    {
        ...
        if (property != null) property.SetValue(owner, value, null);
    ...
    public static Type? GetTypeFromAssemblyQualifiedName(string assemblyQualifiedName)
    {
        return string.IsNullOrEmpty(assemblyQualifiedName)
            ? null
            : GetAllTypes().FirstOrDefault(type => type.AssemblyQualifiedName == assemblyQualifiedName);
    }
    ...
    {
        if (genericDefinition.IsGenericTypeDefinition == false) throw new ArgumentException("genericDefinition has to be generic definition.");
        ...
        while (type != null && type != typeof(object))
        {
            ...
            if (genericDefinition == recursiveType) return true;
    ...
    public static bool IsEmpty(object value)
    {
        ...
        if (value.GetType().GetInterfaces().Contains(typeof(ICanBeEmpty)) && ((ICanBeEmpty)value).IsEmpty()) return true;
