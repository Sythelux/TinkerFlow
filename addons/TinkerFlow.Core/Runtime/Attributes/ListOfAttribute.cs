...
public class ListOfAttribute : MetadataAttribute
{
    ...
    public ListOfAttribute(params Type[] childAttributes)
    {
        ...
        if (uniqueTypes.Length != childAttributes.Length) GD.PushError("Child attributes of ListOf attribute have to be unique. Duplicates are omitted.");
    ...
    {
        var listOfMetadata = (Metadata)metadata;

        if (AreSetsTheSame(childAttributes, listOfMetadata.ChildAttributes, attribute => attribute.Name) == false) return false;
        ...
        {
            if (childAttributes.Any(attribute => attribute.Name == childAttribute.Name) == false) return false;

            if (entryMetadata.ContainsKey(childAttribute.Name) == false) return false;

            if (childAttribute.IsMetadataValid(entryMetadata[childAttribute.Name]) == false) return false;
        }
