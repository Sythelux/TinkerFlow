...
public class DefaultStepLockHandling : StepLockHandlingStrategy
{
    ...
    public override void Unlock(IStepData data, IEnumerable<LockablePropertyData> manualUnlocked)
    {
        ...
        foreach (LockablePropertyData lockable in unlockList) lockable.Property.RequestLocked(false, data);
    }
    ...
    {
        if (completedTransition.Data.TargetStep != null) return completedTransition.Data.TargetStep.Data;

        if (ProcessRunner.IsRunning == false) return null;

        IProcessData process = ProcessRunner.Current.Data;
        // Test all chapters, but the last.
        for (var i = 0; i < process.Chapters.Count - 1; i++)
            if (process.Chapters[i] == process.Current)
            {
                if (process.Chapters[i + 1].Data.FirstStep != null) return process.Chapters[i + 1].Data.FirstStep.Data;
                break;
            }
    ...
    {
        if (mode.ContainsParameter<bool>("LockOnProcessStart")) lockOnProcessStart = mode.GetParameter<bool>("LockOnProcessStart");

        if (mode.ContainsParameter<bool>("LockOnProcessFinished")) lockOnProcessFinished = mode.GetParameter<bool>("LockOnProcessFinished");
    }
