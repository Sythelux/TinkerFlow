...
public partial class Transition : CompletableEntity<Transition.EntityData>, ITransition, ILockablePropertiesProvider
{
    ...
    public class EntityData : EntityCollectionData<ICondition>, ITransitionData
    {
        ...
        [Foldable]
        [ReorderableListOf(typeof(FoldableAttribute), typeof(DeletableAttribute), typeof(HelpAttribute))]
        [ExtendableList]
        ...
        public string Name
        {
            get
            {
                ...
                if (condition != null)
                {
                    ...
                    if (conditionName.Length > 32) conditionName = $"{conditionName.Remove(32)}...";
    ...
    private class EntityAutocompleter : Autocompleter<EntityData>
    {
        ...
        {
            foreach (ICondition condition in Data.Conditions.Where(condition => Data.Mode.CheckIfSkipped(condition.GetType()) == false)) condition.Autocomplete();
        }
    }

    ///<inheritdoc />
    ITransitionData IDataOwner<ITransitionData>.Data => Data;
    ...
    public Transition()
    {
        ...
        if (LifeCycleLoggingConfig.Instance.LogTransitions)
            LifeCycle.StageChanged += (sender, args) => { GD.Print($"{ConsoleUtils.GetTabs(3)}<b>Transition to</b> <i>{(Data.TargetStep != null ? Data.TargetStep.Data.Name + " (Step)" : "chapter's end")}</i> is <b>{LifeCycle.Stage}</b>.\n"); };
    }
    ...
    {
        var clonedTransition = new Transition();
