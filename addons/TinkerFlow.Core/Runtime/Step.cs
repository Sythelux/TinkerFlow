...
public partial class Step : Entity<Step.EntityData>, IStep
{
    ...
    {
        private readonly IEnumerable<LockablePropertyData> toUnlock = new List<LockablePropertyData>();
        ...
        public override IEnumerator Update()
        {
            while (Data.Transitions.Data.Transitions.Any(transition => transition.IsCompleted) == false) yield return null;
        }
    ...
    public IStep Clone()
    {
        var clonedStep = new Step(Data.Name);
    ...
    public Step(string name)
    {
        ...
        if (LifeCycleLoggingConfig.Instance.LogSteps) LifeCycle.StageChanged += (sender, args) => { GD.Print(string.Format("{0}<b>Step</b> <i>'{1}'</i> is <b>{2}</b>.\n", ConsoleUtils.GetTabs(), Data.Name, LifeCycle.Stage)); };
    }
}