...
namespace VRBuilder.Core.Editor.ProcessAssets
{
    ...
    internal static class ProcessAssetUtils
    {
        ...
        {
            var processesDirectory = new DirectoryInfo($"{Application.streamingAssetsPath}/{EditorConfigurator.Instance.ProcessStreamingAssetsSubdirectory}");
            return processesDirectory.GetDirectories()
                .Select(directory => directory.Name)
                .Where(processName => FileAccess.FileExists(GetProcessAssetPath(processName)))
                .ToList();
        }
        ...
        {
            var processesDirectory = new DirectoryInfo($"{Application.streamingAssetsPath}/{EditorConfigurator.Instance.ProcessStreamingAssetsSubdirectory}");
            if (processesDirectory.Exists == false) return false;
        ...
        {
            return FileAccess.FileExists(GetProcessAssetPath(processName));
        }
