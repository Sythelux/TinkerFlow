...
namespace VRBuilder.Core
{
    ...
    public static partial class ProcessRunner
    {
        ...
        private partial class ProcessRunnerInstance : Node
        {
            /// <summary>
...
#elif GODOT
            private void HandleModeChanged(ModeChangedEventArgs args)
#endif
            {
                if (process != null)
                {
...
#elif GODOT
                    RuntimeConfigurator.Instance.ModeChanged -= HandleModeChanged;
#endif
            ...
            public override void _Process(double delta)
            {
                ...
                if (process.LifeCycle.Stage == Stage.Inactive) return;
                ...
                if (currentChapterStage.GetValueOrDefault() != Stage.Activating && process.Data.Current?.LifeCycle.Stage == Stage.Activating) Events.ChapterStarted?.Invoke(this, new ProcessEventArgs(process));

                if (currentStepStage.GetValueOrDefault() != Stage.Activating && process.Data.Current?.Data.Current?.LifeCycle.Stage == Stage.Activating) Events.StepStarted?.Invoke(this, new ProcessEventArgs(process));
        ...
        private static void OnSceneUnloaded(Node scene)
        {
            events = null;
            SceneManager.sceneUnloaded -= OnSceneUnloaded;
        }
        ...
        /// </summary>
        public static IProcess Current => instance == null ? null : instance.process;

        /// <summary>
        /// Returns true if the current <see cref="IProcess"/> is running.
        /// </summary>
        public static bool IsRunning => Current != null && Current.LifeCycle.Stage != Stage.Inactive;
        ...
        {
            var gameObject = new Node();
            gameObject.Name = "PROCESS_RUNNER";
            instance ??= gameObject.AddComponent<ProcessRunnerInstance>();
        ...
        public static void SkipChapters(int numberOfChapters)
        {
            ...
            foreach (IChapter currentChapter in chapters.Skip(chapters.IndexOf(Current.Data.Current)).Take(numberOfChapters)) currentChapter.LifeCycle.MarkToFastForward();
        }
        ...
        public static void SkipCurrentChapter()
        {
            ...
            if (currentChapter.LifeCycle.Stage == Stage.Inactive) currentChapter.LifeCycle.Activate();
